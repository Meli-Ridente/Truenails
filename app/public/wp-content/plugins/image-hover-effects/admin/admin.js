jQuery(document).ready(function($) {
  ...
  jQuery('#la-saved').hide();
     ...
     var parent = jQuery(this).closest('.ui-accordion-content').find('.image');
        ...
        la_caption_hover.on( 'select', function() {
            ...
            selection.map( function( attachment ) {
                ...
                parent.html('<span><img src="'+attachment.url+'"><span class="dashicons dashicons-dismiss"></span></span>');
    ...
    jQuery('#caption').on('click', '.moreimg,.addimgtop', function() { 
        ...
        jQuery('#caption').on('click', '.addcat,.addcattop', function() { 
            sCounter++;
    ...
    // Removing Category
        jQuery('#caption #faqs-container').on('click', '.removecat,.removecattop', function(event) {
    ...
    jQuery('#caption').on('click','.removeitem,.removeimgtop',function() {
    ...
    jQuery('#caption').on('click', '.save-meta', function(event) {
        ...
        var allcats = []; 
          jQuery('.accordian>.content').each(function(index,val) {
            var cats = {};
            cats.cat_name = sanitizeHTML(jQuery(this).find('.catname').val());
            cats.shortcode = jQuery(this).find('.fullshortcode').attr('id');
            cats.allcapImages = [];
            jQuery(this).find('.ui-accordion-content').each(function(index, val) {
                var images = {};
                images.img_name = sanitizeHTML(jQuery(this).find('.imgname').val());
                images.cat_name = sanitizeHTML(jQuery(this).find('.catname').val());
                images.cap_img = jQuery(this).find('img').attr('src');
                images.cap_head = sanitizeHTML(jQuery(this).find('.capheading').val());
                images.cap_desc = sanitizeHTML(jQuery(this).find('.capdesc').val());
                images.cap_link = sanitizeHTML(jQuery(this).find('.caplink').val());
                images.cap_new_tab = jQuery(this).find('.capnewtab').val();
    ...
    jQuery('#caption').on('click','button.fullshortcode,.topshortcode',function(event) {
        event.preventDefault();
